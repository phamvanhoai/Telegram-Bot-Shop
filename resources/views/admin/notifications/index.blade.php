...
<div class="grid gap-8 xl:grid-cols-[minmax(0,1fr)_minmax(500px,1.3fr)]">
 <form method="post" action="{{ route('admin.notifications.store') }}" class="rounded-2xl border border-white/10 bg-slate-900 p-6">@csrf
  ...
  <div class="mt-6 rounded-xl border border-amber-400/20 bg-amber-400/5 p-4 text-sm text-amber-200">Thông báo sẽ được xếp hàng gửi cho toàn bộ người dùng không bị khóa. Hãy kiểm tra kỹ trước khi gửi.</div>
 ...
 @forelse($broadcasts as $item)<div class="rounded-2xl border border-white/10 bg-slate-900 p-5"><div class="flex items-start justify-between gap-4"><div><h3 class="font-bold">{{ $item->title }}</h3><p class="mt-1 text-sm text-slate-500">{{ $item->created_at->format('d/m/Y H:i') }}</p></div><span class="rounded-full px-3 py-1 text-xs {{ $item->completed_at?'bg-emerald-400/10 text-emerald-300':'bg-cyan-400/10 text-cyan-300' }}">{{ $item->completed_at?'HOÀN TẤT':'ĐANG GỬI' }}</span></div><p class="mt-4 line-clamp-3 text-sm text-slate-300">{{ strip_tags($item->message) }}</p><div class="mt-4 flex gap-5 text-sm"><span>Người nhận <b>{{ $item->recipient_count }}</b></span><span class="text-emerald-300">Đã gửi <b>{{ $item->sent_count }}</b></span><span class="text-rose-300">Lỗi <b>{{ $item->failed_count }}</b></span></div></div>
